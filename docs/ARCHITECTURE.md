...
**Modules**: `app.py` (thin navigation shell), `src/dashboard/pages/` (Home, Upload, Prepare, Train, Predict, History — one render function each), `src/dashboard/` support modules (`feature_inputs.py`, `model_status.py`, `history.py`, `reports.py`, `session.py`) — all pure Python where possible, independently unit-testable, none reimplement inference/preprocessing/risk logic.
**Responsibility**: a dataset-driven product workflow (Phase 9, see `docs/PHASE_9_PRODUCT_REDESIGN.md`) — upload a CSV, map its columns to the feature contract, build the leakage-safe target (via `ml.dataset_adapter`, itself calling `network.targets`/`network.validation` unmodified), train the existing model registry (via `ml.training_orchestration`, calling `ml.split`/`ml.train`/`ml.artifacts` unmodified), explicitly save a production model (never automatic, always archiving any prior one), then predict (batch CSV or the original manual-entry form) and view history. Manages PRODUCTION/DEMONSTRATION mode selection (never silently falls back), an in-session prediction history (bounded, never persisted to disk, with an optional user-recorded "actual outcome" for honest actual-vs-predicted comparison), and reads (never recomputes) any real Phase 4 CLI-produced report artifacts under `reports/modeling/`.
